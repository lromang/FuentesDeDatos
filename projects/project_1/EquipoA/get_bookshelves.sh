...
env_dir=$1
texts=0
bookshelves_dir="$env_dir/bookshelves"		#Esta es una carpeta
bookshelves="$bookshelves_dir/bookshelves.txt"	#Este es un archivo de texto
# Print bookshelves
cat "$bookshelves"
echo "Please select a bookshelf id: "
# The user needs to provide at least 2 bookshelfs.
while [ $texts -lt 2 ] || [ ! -z $bookshelf ]		#lt means less than. Esto pregunta mientras tenga menos de dos bookshelves o bookshelves 
							#no esté vacío.
do
    read bookshelf
    # If there are more than two bookshelves selected
    # and bookshelf is empty, we exit the program.
    if [ $texts -gt 2 ] && [ -z $bookshelf ]		#gt means greater than. Esto pregunta si texts es mayor que dos y bookshelves está vacío
    then
       break
    fi
    # ----------------------------------------
    ...
    then
        ...
        then
            ...
            ./get_bookshelves_titles.sh $bookshelf_url $bookshelves_dir
            sleep 1
