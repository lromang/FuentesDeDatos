...
do
    ...
    ###################################################
    rand=$((1+$RANDOM%$n))
    book_url="$(cat $env_dir/bookshelves/$bookshelf_id/titles.txt|awk '{print $1}'|sed -n $((rand))p)"
    book_id="$(cat $env_dir/bookshelves/$bookshelf_id/titles.txt|awk '{print $1}'|sed -n $((rand))p|grep -Eo '[0-9]+'|uniq)"
