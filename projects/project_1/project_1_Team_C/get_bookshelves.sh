...
do
    ...
    # to the selected id.
    # ----------------------------------------
    bookshelf_url=$(cat $bookshelves | grep $bookshelf | awk '{print $1}' |sed '/.*/i https:\/\/www\.gutenberg\.org\/ebooks\/bookshelf\/' |  tr '\n' '\t' | awk '{$NF=$1 $NF}1'|grep -Eo 'https:\/\/www.gutenberg.org\/ebooks\/bookshelf\/[0-9]+')
    echo "$bookshelf_url"
    if [ ! -z $bookshelf_url ]
    then
        if [ $texts -eq 0 ]
