...
class SentClass:
    ...
    def read_data(self, sent_col, pos_label='positive', neg_label='negative'):
        ...
        :return:
        '''
        self.data = (pd.read_csv(os.path.join(self.data_path, 'datosPrueba.csv'))
                     .assign(clean_sentiment=lambda df: df[sent_col].map({pos_label: 1, neg_label: 0})))
        return self.data.copy()
        
    def word_cleaner(self,lista):
        res = []
        for x in lista:
            if x in self.voc:
                res.append(x)
    ...
    def clean_text_df(self, text_col):
        ...
        self.data = (self
                     .data
                     .assign(clean_text=lambda df: proc_text_df(df, text_col))
    ...
    def clean_text(self, text_col):
        ...
        [10 points]
        '''
        self.data[text_col] = self.data[text_col].str.split()
                
        self.data = (self
                     .data
                     .assign(clean_text= lambda df: proc_text(df[text_col],self.sw))
                     .rename({'clean_text': f'words'}, axis=1))
        return self.data
        
    def gen_voc(self, min_count=2):
        '''
        ...
        palabras,c = np.unique(np.array(palabras),return_counts=True)
        
        vocab = pd.Series(c, index = palabras)
        
        self.voc = vocab[vocab>min_count].sort_index()

        # Filter words only those in voc.
        self.data['words'] = (self
                              .data['words']
                              .apply(self.word_cleaner))
        return self.data
    ...
    def gen_voc_features(self, feature_col):
        ...
        '''
        w_counts = (self.data
                    .groupby('sentiment')[feature_col]
                    .apply(list)
                    .map(lambda x: [tuple(y) if type(y) is list else y for z in x for y in z])
                    .reset_index())
    ...
    def build_data_features(self):
        ...
        [10 points]
        '''        
        self.data['bigrams'] = (self
                                .data['words']
                                .apply(get_bigrams))
        
        self.data['neg_words'] = (self
                                  .data['words']
                                  .apply(prefix_neg))
        
        
        self.data['neg_bigrams'] = (self
                                    .data['neg_words']
                                    .apply(get_bigrams))
    ...
    def get_feature_voc(self):
        ...
        for i, feature in enumerate(feature_cols):
            ...
            self.feature_voc[f'f{i}']['pos'] = pos_voc
            self.feature_voc[f'f{i}']['neg'] = neg_voc
        
        return self.feature_voc
    ...
    def text_to_features(self, text):
        ...
        neg_words = prefix_neg(words[0])
        data_features = {'f0': words[0], 'f1': get_bigrams(words[0]),
...
if __name__ == '__main__':
    ...
    crudos = sc.read_data('sentiment')
    data = sc.clean_text('review')
    datos = sc.build_data_features()
    nuevos = sc.gen_voc()
    features = sc.get_feature_voc()
