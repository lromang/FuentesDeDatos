...
#         HINT 2: what is this expression doing: ^(.*)\.txt$
		#Hace match con cualquier expresión que comience como
		#sea y termine en ".txt". Agrupa el comienzo hasta
		#que encuentra la terminación del archivo
#   2.2.- What does the option -n do?
		#En vez de ponernos el texto completo y resaltar los resultados
		#nos regresa exactamente todos los matches
#         g? aplica los cambios de manera global
#         p? imprime.
# ----------------------------------------
for team in $(ls $team_dir | gsed -nE 's/^(.*)\.txt$/\1/gp)
