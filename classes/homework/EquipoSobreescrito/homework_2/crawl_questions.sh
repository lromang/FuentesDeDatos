...
#	Nope, because the normal questions are formatted VERY 
#	differently under h3 headers. Hot network ones are under
#	h4.
...
cat all_questions.txt|sed -nE '/<h3><a href=\"\/questions\//p' | grep -oE '>[[:alnum:]].+</a' | sed -E 's/>//g ; s/<\/a//g' > all_questions_filtered.txt

cat all_questions_filtered.txt
